...
class Compare:
 ...
 def comparing(self):
  ...
  print "Total number of lines starting with 'tempest' in dest file:'",length2                       #2059
  ...
  for key in self.dict1:
    for jey in self.dict2:
       if(key==jey):
          if((self.dict1[key]!='ok')&(self.dict1[key]!='FAIL'))&((self.dict2[jey]!='ok')&(self.dict2[jey]!='FAIL')):
             # print key,self.dict1[key]
              count3=count3+1
              self.f2.writelines(key+'\n')
  print "Total number of lines from 'tempest' to '...' matching status anything",count3                      #87
  self.f2.close()   
